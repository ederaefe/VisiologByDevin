...
export function createExtractionPrompt(template = DEFAULT_TEMPLATE) {
    ...
    const header = activeTemplate.fields.map(field => field.key).join(',');
...
Return raw CSV. The first line must be this exact header row, with fields in this order:
${header}
Then return one CSV line per data row. Skip the logbook's header row and fully blank rows.

Template fields:
${definitions}

Use standard RFC4180 CSV quoting: wrap a value in double quotes when it contains a comma,
double-quote, or newline, and escape embedded double-quotes by doubling them.
Use an empty cell for missing or unreadable values. Never invent, infer, silently correct,
or combine values. Preserve values as written.
Return ONLY raw CSV: no markdown, no code fences, no commentary, and no confidence data.
`;
}
